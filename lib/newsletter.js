...
async function handleNewsletterSubscribe(req, res) {
  ...
  const confirmUrl = `${getSiteBaseUrl(req)}/newsletter-confirm.html?token=${encodeURIComponent(token)}`;
  const text = [
    `Olá, ${nome}!`,
    '',
...
function handleNewsletterConfirmPage(req, res) {
  ...
  return res.redirect(302, `/newsletter-confirm.html?token=${encodeURIComponent(token)}`);
}
