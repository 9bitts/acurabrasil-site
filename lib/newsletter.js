...
async function handleNewsletterSubscribe(req, res) {
  ...
  const confirmUrl = `${getSiteBaseUrl(req)}/api/newsletter/confirm?token=${encodeURIComponent(token)}`;
  const text = [
    `Olá, ${nome}!`,
    '',
    'Obrigado por se inscribir na newsletter da ACURA BRASIL.',
  ...
  try {
    await withTimeout(
      sendNewsletterEmail({
        to: email,
        subject: 'Confirme sua inscrição — ACURA BRASIL',
        text,
      }),
      SEND_TIMEOUT_MS,
      'NEWSLETTER_SEND'
...
function handleNewsletterConfirm(req, res) {
  ...
  if (!token || token.length < 32) {
    return res.redirect(302, '/index.html?newsletter=invalid');
  }

  const db = getDb();
  const row = db.prepare('SELECT id, confirmed FROM newsletter_subscribers WHERE confirm_token = ?').get(token);
  if (!row) {
    return res.redirect(302, '/index.html?newsletter=invalid');
  }
  ...
  return res.redirect(302, '/index.html?newsletter=confirmed');
}
