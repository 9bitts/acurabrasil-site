...
/** Set MASTERCLASS_EFT_REGISTRATIONS_CLOSED=1 to close the public form/API. */
const RATE_LIMIT_MS = 60_000;
const rateLimit = new Map();

function areRegistrationsClosed() {
  return process.env.MASTERCLASS_EFT_REGISTRATIONS_CLOSED === '1';
}
...
async function handleMasterclassRegister(req, res) {
  if (areRegistrationsClosed()) {
    return res.status(403).json({ ok: false, error: 'registrations_closed' });
  }
