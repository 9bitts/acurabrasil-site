...
function getSessionSecret() {
  ...
  if (!devSessionSecret) {
    ...
    console.warn(
      'ADMIN_SESSION_SECRET not set — using ephemeral random secret (admin sessions reset on every restart)'
    );
  }
...
function validateAdminAuthConfig() {
  ...
  if (isProd) {
    ...
    if (!process.env.ADMIN_PASSWORD_HASH) {
      throw new Error(
        'ADMIN_PASSWORD_HASH must be set in production (generate with: node scripts/hash-password.js)'
      );
    }
    return;
  }
  ...
  if (process.env.ADMIN_PASSWORD) {
    console.warn(
      'ADMIN_PASSWORD is set in plain text (dev only) — use ADMIN_PASSWORD_HASH in production'
    );
    return;
  }
  console.warn('No admin password configured — admin login will fail until ADMIN_PASSWORD is set');
}
