...
Site: https://www.acurabrasil.org/atendimento-humanitario
...
function migrate(db) {
  ...
  if (current < 27) applyMigrationV27(db);
  if (current < 28) applyMigrationV28(db);
  if (current < 29) applyMigrationV29(db);
}
...
function applyMigrationV27(db) {
  db.exec(`
    ALTER TABLE masterclass_registrations ADD COLUMN confirmation_email_sent_at TEXT;
    ALTER TABLE masterclass_registrations ADD COLUMN last_email_sent_at TEXT;
    ALTER TABLE masterclass_registrations ADD COLUMN doctor8_checked_at TEXT;
    ALTER TABLE masterclass_registrations ADD COLUMN doctor8_status TEXT NOT NULL DEFAULT '';
    ALTER TABLE masterclass_registrations ADD COLUMN doctor8_registered INTEGER NOT NULL DEFAULT 0;
    ALTER TABLE masterclass_registrations ADD COLUMN doctor8_profile_json TEXT NOT NULL DEFAULT '';
  `);
  console.log('Migration v27: masterclass email confirmation + Doctor8 lookup fields');
  setSchemaVersion(db, 27);
}

function applyMigrationV28(db) {
  db.exec(`
    ALTER TABLE masterclass_registrations ADD COLUMN doctor8_approval_status TEXT NOT NULL DEFAULT '';
    ALTER TABLE masterclass_registrations ADD COLUMN doctor8_approved INTEGER NOT NULL DEFAULT 0;
    ALTER TABLE masterclass_registrations ADD COLUMN doctor8_documents_verified INTEGER NOT NULL DEFAULT 0;
  `);
  console.log('Migration v28: masterclass Doctor8 approval/document status');
  setSchemaVersion(db, 28);
}

function applyMigrationV29(db) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_course_enrollments_verify ON course_enrollments(verify_code);
  `);
  console.log('Migration v29: courses marketplace (free for volunteers)');
