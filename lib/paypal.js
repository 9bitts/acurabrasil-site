...
async function createSubscriptionPlan(amount, cause) {
  const cacheKey = `${amount}:${cause}`;
  if (PLAN_CACHE.has(cacheKey)) {
    return PLAN_CACHE.get(cacheKey);
  }

  const causeLabel =
    cause === 'pesquisa' ? 'Pesquisa Científica ACURA' : 'Atendimento Humanitário ACURA';
  const value = amount.toFixed(2);

  const product = await paypalFetch('/v1/catalogs/products', {
    method: 'POST',
    body: JSON.stringify({
      name: `Doação Mensal ACURA BRASIL — ${causeLabel}`,
      description: `Doação recorrente mensal de R$ ${value} para ACURA BRASIL (OSCIP)`,
      type: 'SERVICE',
      category: 'CHARITY',
    }),
  });
  ...
  const plan = await paypalFetch('/v1/billing/plans', {
    ...
    body: JSON.stringify({
      ...
      name: `ACURA Mensal R$ ${value}`,
      description: `Doação mensal R$ ${value} — ${causeLabel}`,
      billing_cycles: [
        {
...
async function handleSubscriptionPlan(req, res) {
  ...
  try {
    const planId = await createSubscriptionPlan(amount, cause);
