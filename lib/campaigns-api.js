  ...
  app.post('/api/campaigns/:slug/donations', (req, res) => {
    try {
      ...
      const donation = createDonation(campaign.id, {
        ...(req.body || {}),
        method: req.body?.method === 'paypal' ? 'paypal' : req.body?.method || 'paypal',
        status: req.body?.status === 'confirmed' ? 'confirmed' : 'reported',
        bump_totals: true,
      });
      const updated = getCampaignBySlug(req.params.slug);
      res.status(201).json({ ok: true, donation, campaign: updated });
    } catch (err) {
      handleError(res, err);
    }
  });
