...
function findRecentIntakeByClientRequestId(clientRequestId) {
  if (!clientRequestId) return null;
  const db = getDb();
  return db
    .prepare(
      `SELECT protocolo FROM sos_intakes
...
function persistIntake(data) {
  ...
  const insert = db.prepare(`
    INSERT INTO sos_intakes (
      ...
      lgpd_privacy_accepted, lgpd_privacy_version, lgpd_privacy_at, client_request_id
    ) VALUES (
      @protocolo, @intake_token_hash, @nome, @email, @phone_json, @relacion, @nome_paciente, @edad,
  ...
  try {
    const { schedulePush } = require('./doctor8-sync');
    schedulePush(data.protocolo);
  } catch (syncErr) {
    console.error('Doctor8 sync schedule failed:', syncErr.message);
  }
...
module.exports = {
  persistIntake,
  findRecentIntakeByClientRequestId,
  verifyIntakeToken,
  hashIntakeToken,
  generateIntakeToken,
};
