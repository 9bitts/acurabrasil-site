...
function getBaseUrl() {
  return sanitizeEnvValue(process.env.DOCTOR8_API_BASE_URL).replace(/\/$/, '');
}

function getApiKey() {
  return sanitizeEnvValue(process.env.DOCTOR8_API_KEY);
}
...
function buildLookupResult({ registered, user, email, source, approval, rawData }) {
  ...
  const merged =
    approval && approval.approvalStatus && approval.approvalStatus !== 'unknown'
      ? approval
      : fromPayload;
...
function formatFetchError(err) {
  ...
  if (cause) {
    ...
    if (cause.message && cause.message !== err.message) parts.push(String(cause.message));
  }
  return parts.join(' | ') || 'unknown';
}

function doctor8HostLabel() {
  try {
    const base = getBaseUrl();
    return base ? new URL(base).host : '(missing-base-url)';
  } catch {
...
async function doctor8Request({ path, method = 'POST', email, body }) {
  ...
  if (!baseUrl) {
    const err = new Error('DOCTOR8_API_BASE_URL missing or invalid');
  ...
  const headers = {
    ...
    'User-Agent': 'AcuraBrasil-Integrations/1.0',
  };
...
async function checkEmailRegistered(email) {
  ...
  try {
    ...
    if (!res.ok) {
      const detail = `HTTP ${res.status} host=${doctor8HostLabel()} path=${pathTemplate}`;
      console.error(`[doctor8-api] check-email ${detail}`, `email=${normalized}`);
      return {
        ok: false,
        configured: true,
        status: 'error',
    ...
    const detail = `${formatFetchError(err)} host=${doctor8HostLabel()} path=${pathTemplate}`;
    console.error(`[doctor8-api] check-email ${kind}:`, detail, `email=${normalized}`);
    return {
      ok: false,
      configured: true,
      status: 'error',
...
async function lookupUserByEmail(email) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.warn(
        `[doctor8-api] lookup-user HTTP ${res.status}`,
        `host=${doctor8HostLabel()}`,
        `path=${lookupPath}`,
        `email=${normalized}`
      );
    }
    ...
    if (err.name === 'AbortError' && !check.ok) {
      return {
        ...
        status: 'error',
        error: 'timeout',
        detail: `${formatFetchError(err)} host=${doctor8HostLabel()} path=${lookupPath}`,
        source: 'lookup-user',
        user: null,
        email: normalized,
        approvalStatus: 'unknown',
