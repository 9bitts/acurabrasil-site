  ...
  const result = db
    .prepare(
      `INSERT INTO campaigns (
        ...
        cover_url, video_url, gallery_json,
...
function updateCampaign(id, body) {
  ...
  const data = normalizePayload({ ...existing, ...body, gallery: body.gallery ?? parseJsonField(existing.gallery_json, []), suggested_amounts: body.suggested_amounts ?? parseJsonField(existing.suggested_amounts_json, DEFAULT_SUGGESTED) }, { partial: false });
  ...
  db.prepare(
    `UPDATE campaigns SET
      ...
      gallery_json=@gallery_json, goal_amount=@goal_amount, raised_amount=@raised_amount,
...
Cada aporte fortalece una red de cuidado que une a ACURABRASIL y la plataforma Doctor8.`,
    ...
    secondary_cta_label_pt: 'Quero ser voluntário',
    secondary_cta_label_es: 'Quiero ser voluntario',
    secondary_cta_url: '/voluntarios',
...
Não há prazo de encerramento. Você pode doar uma vez ou mensalmente via Pix ou PayPal.`,
...
No hay plazo de cierre. Puedes donar una vez o mensualmente vía Pix o PayPal.`,
