...
function rowToCampaign(row, { includeInternal = false } = {}) {
  ...
  const campaign = {
    ...
    min_amount: num(row.min_amount, 1),
...
function normalizePayload(body, { partial = false } = {}) {
  ...
  for (const key of numFields) {
    if (body[key] != null || !partial) {
      out[key] = num(body[key], key === 'min_amount' ? 1 : key === 'max_amount' ? 50000 : 0);
    }
  }
...
function createDonation(campaignId, body) {
  ...
  // PayPal já capturou o pagamento no cliente — não rejeitar valores baixos legítimos.
  const paypalConfirmed =
    body.method === 'paypal' && body.status === 'confirmed' && amount >= 1;
  const minAllowed = paypalConfirmed ? 1 : campaign.min_amount;
  if (amount < minAllowed || amount > campaign.max_amount) {
...
function seedDefaultCampaigns(db) {
  ...
  // Campanhas padrão ficam em rascunho; só Impressos (migration v16/v19) fica publicada no hub.
  insert.run({
    slug: 'sos-venezuela',
    type: 'emergency',
    status: 'draft',
