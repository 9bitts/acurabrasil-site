(function () {
  ...
  const renderCard = (prof) => {
    ...
    const photoUrl = safeHttpUrl(prof.photo);
    const photoHtml = photoUrl
      ? `<img src="${escapeHtml(photoUrl)}" alt="" class="consulta-prof-photo" loading="lazy" width="96" height="96">`
    ...
    if (agendamentoUrl) {
      actions.push(
        `<a href="${escapeHtml(agendamentoUrl)}" class="consulta-prof-action" target="_blank" rel="noopener" title="${escapeHtml(t('consulta.prof.btn.whatsapp'))}">
